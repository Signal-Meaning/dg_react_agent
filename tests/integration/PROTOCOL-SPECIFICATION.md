...
| **conversation.item.input_audio_transcription.completed** | Map to **Transcript** (is_final: true). Clear delta accumulator for that item_id (Issue #497). | (covered by transcription flow) |
...
| Transcript | Text | From conversation.item.input_audio_transcription.completed (final) or .delta (interim, accumulated per item_id, Issue #497) | `Issue #497: input_audio_transcription.delta accumulated per item_id → Transcript with accumulated text` |
