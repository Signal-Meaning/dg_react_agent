...
| **conversation.item.created** / **.added** / **.done** | Decrement pending-item counter; when 0, send `response.create` to upstream. Forward event to client as **text**. | `Issue #388: sends response.create only after receiving conversation.item.added from upstream for InjectUserMessage`; `Protocol: same item id .added + .done count once` |
| **response.output_text.done** | Clear response-in-progress; if deferred after FunctionCallResponse, send `response.create` to upstream. Send **AgentStartedSpeaking** (if not yet sent), then **ConversationText** (assistant), then **AgentAudioDone**; flush any buffered idle_timeout Error. | `Issue #462: does not send session.update after output_audio.done until output_text.done`; `Issue #482 TDD: client receives AgentAudioDone when response completes (output_text.done)`; `Issue #482: client receives ConversationText (assistant) before Error (idle_timeout) when upstream sends error before output_text.done` |
...
| **response.output_audio_transcript.done** | Map to **ConversationText** (assistant); send as **text**. | (covered by general ConversationText / completion tests) |
| **response.function_call_arguments.done** | Map to **FunctionCallRequest** and **ConversationText** (assistant); send both as **text**. | `translates response.function_call_arguments.done to FunctionCallRequest...`; `sends FunctionCallRequest then ConversationText when upstream sends response.function_call_arguments.done` |
...
| ConversationText (assistant) | Text | From output_text.done, output_audio_transcript.done, function_call_arguments.done, or greeting | `translates InjectUserMessage to conversation.item.create + response.create and response.output_text.done to ConversationText`; Issue #489 real-API InjectUserMessage/FunctionCallResponse tests |
