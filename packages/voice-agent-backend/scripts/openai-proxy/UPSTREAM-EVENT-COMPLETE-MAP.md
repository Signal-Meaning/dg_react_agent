...
| **conversation.item.created** / **.added** / **.done** | `type`, `item?: { id?, type?, role?, content? }`. Proxy uses `item.id` for pending-item counter and dedupe; `item.role`, `item.content` for **ConversationText** (assistant). **Issue #522:** When `msg.type === 'conversation.item.done'` and `item.type === 'function_call_output'` and we deferred after FunctionCallResponse, send deferred **response.create** (per spec: item.done = "item finalized"; API may not send response.done after function_call_output). See REQUIRED-UPSTREAM-CONTRACT.md. | Decrement counter; if 0 send response.create. If assistant, send **ConversationText** (from content, including function_call parts). Do not forward raw event. | Yes |
