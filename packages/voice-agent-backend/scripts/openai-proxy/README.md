...
**TLS (EPIC-546):** Generic `HTTPS=true` does **not** enable TLS in `run.ts`. Use **`OPENAI_PROXY_TLS_KEY_PATH`** + **`OPENAI_PROXY_TLS_CERT_PATH`** (PEM files), or **`OPENAI_PROXY_INSECURE_DEV_TLS=1`** for dev self-signed (not in production). When spawned via **`attachVoiceAgentUpgrade`**, the parent sets the insecure flag when it serves `wss` unless PEM paths are supplied.
