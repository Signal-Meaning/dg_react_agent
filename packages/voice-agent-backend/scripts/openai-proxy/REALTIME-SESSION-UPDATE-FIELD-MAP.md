...
| `audio` | **Input:** GA shape (PCM 24kHz, transcription). **Default** **`turn_detection.type: server_vad`** via **`buildOpenAIServerVadTurnDetection(agent.idleTimeoutMs)`** (VAD defaults + **`idle_timeout_ms`** clamped 5s–30s; `-1`/omit → `null` on `idle_timeout_ms`); proxy mic path is **append-only** (no proxy `input_audio_buffer.commit`). **Internal / Jest only:** **`agent.useOpenAIManualAudioCommit: true`** → **`turn_detection: null`** (legacy proxy `commit` + `response.create` path). **Output:** merged from `agent.sessionAudioOutput` when set. | Issue #560 Phase 2b + product default Server VAD; #414; #540 output. |
| `agent.useOpenAIManualAudioCommit` | Not sent by **`buildSettingsMessage`**. When `true` on Settings JSON (tests), forces **`turn_detection: null`** and manual commit path. | Jest / integration mocks only. |
...
| `agent.idleTimeoutMs` | **Client idle** (WebSocketManager / idle manager). On **`session.update`:** when using default Server VAD, mapped to **`session.audio.input.turn_detection.idle_timeout_ms`** (clamped; `-1`/invalid → `null` on that field). With **`useOpenAIManualAudioCommit`**, `turn_detection` is null so this is **not** forwarded on the wire — [PROTOCOL-AND-MESSAGE-ORDERING.md §3.9](./PROTOCOL-AND-MESSAGE-ORDERING.md#39-idle-timeout-expected-closure-not-an-error). |
| `Settings.audio` (component encoding / sample_rate) | Used for component audio pipeline metadata; **input** shape on `session.audio` is **proxy-owned** (PCM 24kHz, default **`server_vad`** turn detection, transcription) — not a direct copy of Settings.audio. |
