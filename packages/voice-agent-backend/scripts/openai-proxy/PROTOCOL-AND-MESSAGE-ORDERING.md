...
|------------------------|--------------|
| **Settings** | Map to `session.update`; send to upstream once per connection **only when no response is active** (Issue #459). If a response is in progress, send `SettingsApplied` only (no `session.update`). On duplicate Settings, send `SettingsApplied` only (no second `session.update`). Store context and greeting when session is updated. When set, `agent.think.provider.temperature` maps to Realtime `session.temperature` (Issue #538; [API](https://platform.openai.com/docs/api-reference/realtime-client-events/session/update)). When set, `agent.think.toolChoice` maps to `session.tool_choice` (`auto` \| `none` \| `required` or `{ type: 'function', name }` — Issue #535). When non-empty after validation, `agent.think.outputModalities` maps to `session.output_modalities` (`text` and/or `audio` — Issue #536). When set to a positive safe integer, `agent.think.maxOutputTokens` maps to `session.max_output_tokens` (Issue #537). |
