...
| **E. Issue #482 real-API test** | `expect(errIdx).toBeGreaterThanOrEqual(0)` fails (received -1). Test expects an Error with `SERVER_TIMEOUT_ERROR_CODE`; client never received it. | Real-API/environment: API may not send idle_timeout in the expected window or message order differs. Treat as env-specific; relax or skip when not reproducible. |
