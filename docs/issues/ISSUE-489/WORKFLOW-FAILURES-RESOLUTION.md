# Issue #489: Test and Publish Workflow Failures — Resolution
...
- **Policy:** See [PUBLISHING-AND-RELEASING.md](../../PUBLISHING-AND-RELEASING.md): release flow is create `release/vX.X.X` → create GitHub release (tag from that branch) → CI runs and publishes. CI is intended to run in that context only.
...
| **npm audit** | High/critical vulnerabilities | Run `npm audit --audit-level=high` locally; fix or allow with `npm audit fix` / overrides as per policy. **Prerequisite:** Pass audit in Pre-Release (see [RELEASE-CHECKLIST.md](./RELEASE-CHECKLIST.md)) before triggering the workflow. |
...
| **Verify authentication** | `npm whoami` fails (401) | **NPM_TOKEN** invalid or expired. See [PUBLISHING-AND-RELEASING.md](../../PUBLISHING-AND-RELEASING.md): create/regenerate a PAT with `write:packages`, update the **NPM_TOKEN** repo secret in Settings → Secrets and variables → Actions. |
...
- **Fallback (dev publish):** If CI cannot be fixed quickly, from repo root on the release branch (e.g. `release/v0.9.8`) with `NPM_TOKEN` in env or `.npmrc`: `npm publish`. Then tag and create the GitHub release manually. See [RELEASE-CHECKLIST.md](./RELEASE-CHECKLIST.md) and [PUBLISHING-AND-RELEASING.md](../../PUBLISHING-AND-RELEASING.md).

---

## References

- [RELEASE-CHECKLIST.md](./RELEASE-CHECKLIST.md) — Issue #489 release steps
- [PUBLISHING-AND-RELEASING.md](../../PUBLISHING-AND-RELEASING.md) — NPM_TOKEN, publish flow
- [.github/workflows/test-and-publish.yml](../../../.github/workflows/test-and-publish.yml) — workflow source
- [.github/workflows/README.md](../../../.github/workflows/README.md) — how to run and test the workflow
