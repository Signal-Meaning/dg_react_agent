...
| **3. Proxy sends function_call_output** | Does the proxy, when it receives FunctionCallResponse from the client, send `conversation.item.create` with `type: 'function_call_output'`, correct `call_id`, and `output`? | Covered by **integration tests** (openai-proxy-integration, backend-integration) and by **tests 6/6b outcome** when they pass. The proxy writes to a file when `LOG_LEVEL=debug` (for manual inspection only; test 6d does not assert on it). | If 6/6b fail but 6d steps 1–2 pass, defect is proxy or API; use integration tests and proxy unit tests to verify proxy behavior. |
...
**Minimal instrumentation checklist:** (1) In the app: expose backend request URL, response status, and whether `sendResponse` was called with `result` or `error` (`__functionCallDiagnostics`). (2) In the proxy: when `LOG_LEVEL=debug`, write `function_call_output` to a file for inspection (optional; not asserted). (3) Test 6d asserts steps 1–2 only; run it to pinpoint browser→backend and app→sendResponse.

### Automated diagnostic (test 6d) and result

**Test 6d** runs the same flow as test 6 but after `waitForFunctionCall` reads `window.__functionCallDiagnostics` and network capture for `/function-call`. It asserts **only steps 1–2**: (1) browser→backend: POST to `/function-call`, 200, CORS; (2) app sent sendResponse with result. It does **not** assert that the proxy wrote a log file — asserting "a log exists" is not a behavioral requirement; proxy→upstream is covered by integration tests and by tests 6/6b outcome when they pass.

**Optional debug file:** When the backend runs with `LOG_LEVEL=debug`, the backend sets `E2E_FUNCTION_CALL_DEBUG_LOG` for the proxy subprocess so the proxy writes `function_call_output` to `test-results/e2e-function-call-output.json` for manual inspection. Playwright starts the backend with `LOG_LEVEL=debug` and cwd so that path is test-app/test-results. No test assertion on that file.

**Backend log evidence:** Use `npm run backend:log` (test-app) to run the backend with info logging and write a timestamped log file; logs show POST to `/function-call` and 200 response.
...
- **When user starts backend and frontend (existing servers):** Steps 1–2 **pass**; 6d passes. Evidence: `diagnostics.status` 200, `contentPreview` with time/UTC, network capture shows POST and 200 with CORS. Run: `E2E_USE_EXISTING_SERVER=1 USE_REAL_APIS=1 npm run test:e2e -- openai-proxy-e2e.spec.js --grep "6d"` (from test-app).

### When 6d passes but 6b (or 6) still fails

Steps 1–2 are verified (browser→backend, app sent sendResponse). The failure is **step 3 (proxy→upstream) or step 4 (API/model)**. Next in order:

1. **Confirm proxy sent function_call_output**  
