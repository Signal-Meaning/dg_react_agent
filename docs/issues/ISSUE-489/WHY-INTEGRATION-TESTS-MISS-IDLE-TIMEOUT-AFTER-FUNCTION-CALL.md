...
**Real-API integration test (added, passing):** We **now have** an integration test that runs against the **real** OpenAI API and asserts that the **client** receives AgentAudioDone after sending FunctionCallResponse: `Issue #489 real-API: after FunctionCallResponse client receives AgentAudioDone (proxy received completion)` in `tests/integration/openai-proxy-integration.test.ts`. This test **passes** with USE_REAL_APIS=1, proving that the proxy does receive completion from upstream and sends AgentAudioDone to the client. So "API never sends completion" is ruled out at the wire level. The remaining E2E failure (issue-373 "re-enable idle timeout after function calls complete") is therefore attributed to **component or E2E environment**, not the proxy. See [PROTOCOL-ASSURANCE-GAPS.md](./PROTOCOL-ASSURANCE-GAPS.md) and [E2E-FAILURES-RESOLUTION.md](./E2E-FAILURES-RESOLUTION.md).
...
3. **Integration test (mock):** The OpenAI proxy integration test suite includes a **protocol contract** test (Issue #487): "Within N ms of the client sending FunctionCallResponse, the client receives at least one of: AgentThinking, ConversationText (assistant), or AgentAudioDone." This uses the **mock** upstream. **Integration test (real API, added):** `Issue #489 real-API: after FunctionCallResponse client receives AgentAudioDone (proxy received completion)` runs with USE_REAL_APIS=1 and **passes** — so with the real API the client does receive AgentAudioDone after a function call. That proves the proxy sends completion; the E2E failure is component/E2E env. See `tests/integration/openai-proxy-integration.test.ts`.
