...
**Done:** Test-app now persists `conversationHistory` to `sessionStorage` (key `dg_voice_conversation`, last 50 messages) and restores on mount, so after a full page refresh the conversation is restored and passed as context to the component for both OpenAI and Deepgram. A "Conversation History" section in the UI (`data-testid="conversation-history"`) displays the list so restored messages are visible after refresh.

### Remaining (next steps)

1. **Optional E2E for conversation after refresh:** Add an E2E test that sends a message, refreshes the page, then asserts the conversation is still visible (e.g. `conversation-history` has expected messages). Would validate the persistence path for either backend.
2. **If upstream still closes:** The proxy mitigation (Error `upstream_closed_before_session_ready`) remains. If issues persist with real API after the protocol fix, investigate session.update payload, timeouts, auth, rate limits, or report to OpenAI.
