...
| File | Description |
|------|-------------|
| `tests/integration/openai-proxy-integration.test.ts` | OpenAI proxy integration; when `USE_REAL_OPENAI=1` and `OPENAI_API_KEY` are set, a subset of tests run against the live OpenAI Realtime API. |

## Tests that run when `USE_REAL_OPENAI=1` and `OPENAI_API_KEY` is set

When `useRealOpenAI` is true, mock-only tests (`itMockOnly`) are skipped. The following **8 tests** run and must all pass:
...
| 5 | `Issue #414 real-API: firm audio connection — no Error from upstream within 12s after sending audio (USE_REAL_OPENAI=1)` |
| 6 | `Issue #414 real-API: firm audio (speech-like audio) — no Error from upstream within 12s (USE_REAL_OPENAI=1)` |
...
- `tests/integration/openai-proxy-integration.test.ts` – `useRealOpenAI`, `itMockOnly`, `(useRealOpenAI ? it : it.skip)`.
