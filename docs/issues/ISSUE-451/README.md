...
**In scope for this release:** The tests that run when `USE_REAL_APIS=1` and `OPENAI_API_KEY` are set. Defined in **[SCOPE.md](./SCOPE.md)**:
...
- **Run (real API):** `USE_REAL_APIS=1 npm test -- tests/integration/openai-proxy-integration.test.ts` (requires `OPENAI_API_KEY`).
...
| **2 – Green** | Implement or adjust code/tests so in-scope tests pass with `USE_REAL_APIS=1`. Refactor as needed. | This README + [TRACKING.md](../ISSUE-455/TRACKING.md) |
...
- **[SCOPE.md](./SCOPE.md)** – Phase 1: in-scope test file and list of 8 tests that run with `USE_REAL_APIS=1`.
