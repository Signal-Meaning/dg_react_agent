...
- **Context warning on reconnection:** The component knows first connection vs reconnection (WebSocket manager passes `isReconnection`). When it sends Settings on a **reconnection** and `agentOptions.context` is missing or has no messages, it calls the optional **`onContextWarning`** prop so the app can warn the user or log that the new connection has no conversation context.
