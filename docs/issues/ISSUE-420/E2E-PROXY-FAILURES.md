# E2E Proxy-Mode Failures (11 tests → 9 remaining)
...
| # | Spec | Test | Error |
|---|------|------|--------|
| 1 | declarative-props-api.spec.js:175 | should connect when autoStartAgent is true | Test timeout 60s — `page.waitForFunction` (connection-status / `__testConnectionState === 'connected'`) never satisfied |
...
| 7 | microphone-control.spec.js:68 | should start transcription service when microphone button clicked with agent already connected (Issue #255) | Timeout 30s — `waitForTranscriptionConnected` (transcription-connection-status never "connected") |
...
### A. Declarative Props API (Issue #305) — 6 failures

Tests drive the app via **window globals** (`__testAutoStartAgent`, `__testConnectionState`, `__testConnectionStateSet`, etc.) and expect the DOM to show "connected". The test-app may not wire these globals to the component in the test-mode page, or the declarative API may not be fully implemented, so connection never appears.
...
### C. Microphone / transcription — 1 failure

- **#7:** Test expects **transcription** to show "connected" when mic is clicked with agent already connected. In proxy mode the app may use only the agent channel; transcription-connection-status may never show connected.

**Fix options:** Skip or relax this test when only agent (no transcription) is used; or enable transcription in proxy test config and ensure UI shows it.
...
1. **Triage:** Decide which failures are release-blocking vs. acceptable to skip or document for v0.7.19.
2. **Declarative / test-mode:** Confirm whether test-app exposes `__test*` globals and declarative props in the page used by these E2E tests; implement or skip accordingly.
3. **Proxy vs. transcription:** For microphone-control #255, confirm if transcription is expected in proxy mode and either enable it or skip the assertion.
4. **OpenAI TTS/function-call:** Reproduce #9 and #10 locally; check proxy/backend logs and TTS format; fix or relax assertions.
