...
0. **(agent)** **Commit scheduler + Server VAD migration:** [COMMIT-SCHEDULER-TDD-PLAN.md](./COMMIT-SCHEDULER-TDD-PLAN.md) — **Phase 0 `[x]`**; **Phase 1 RED `[x]`** (scheduler Jest **failing** until Phase 2). **Next:** Phase 2 **GREEN** (`server.ts` + `-t "Issue #560 / scheduler"`), then **Phase 2b** / **P1** / **P2** per **§9**. Hybrid `create_response: false` **discounted** (plan §2).
