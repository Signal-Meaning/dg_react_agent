...
0. **(agent)** **Phase 2b + product default Server VAD (2026-04):** **`mapSettingsToSessionUpdate`** defaults to **`server_vad`** (append-only mic). **`agent.useOpenAIManualAudioCommit`** is **internal / Jest-only** (forces null `turn_detection` + proxy `commit` in mocks). No UI field — **`buildSettingsMessage`** does not send manual-commit. Docs: [COMMIT-SCHEDULER-TDD-PLAN.md](./COMMIT-SCHEDULER-TDD-PLAN.md) **§6b/§9**, [REALTIME-SESSION-UPDATE-FIELD-MAP.md](../../../packages/voice-agent-backend/scripts/openai-proxy/REALTIME-SESSION-UPDATE-FIELD-MAP.md). **Next (P1):** **`USE_REAL_APIS=1`** subset for VAD event order/transcription; Phase **3** doc sweep ([COMMIT-SCHEDULER-TDD-PLAN.md](./COMMIT-SCHEDULER-TDD-PLAN.md) §7). **Verify:** `npm test -- tests/openai-proxy.test.ts`; `npm test -- tests/integration/openai-proxy-integration.test.ts --runInBand --forceExit -t "Phase 2b"`.
