...
0. **(agent)** **Server VAD migration (Phase 2b):** [COMMIT-SCHEDULER-TDD-PLAN.md](./COMMIT-SCHEDULER-TDD-PLAN.md) **§6b** / **§9** — **`mapSettingsToSessionUpdate`** → **`server_vad`** + idle/silence fields; **`server.ts`** — no proxy **`commit`** on VAD mic path; **`response.create`** aligned with PROTOCOL §3.6; mock + real-API tests. **Done (2026-04-04):** Phase **1 RED** + Phase **2 GREEN** (max coalesce + commit-on-close); verify with `npm test -- tests/integration/openai-proxy-integration.test.ts --runInBand --forceExit` and `-t "Issue #560 / scheduler"`. Hybrid `create_response: false` **discounted** (plan §2).
