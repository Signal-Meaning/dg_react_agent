...
| **Isolation trace (round 1)** | **Done** — [#561](../ISSUE-561/README.md), `App.tsx` / `voiceAgentStartOptions.ts` |
...
| Backend: package `.env` + `npm run start`; OpenAI `run.ts` skips `test-app/.env` | **Done** — [ARCHITECTURE.md](../../BACKEND-PROXY/ARCHITECTURE.md) |
| Code trace: E2E `sendAudioData` vs mic → same `sendAudioData` (round 2) | **Done** — [LIVE-MODE-OPENAI-E2E-ISOLATION.md](./LIVE-MODE-OPENAI-E2E-ISOLATION.md) |
...
| Real-API re-qualify (OpenAI proxy E2E + Live, `USE_REAL_APIS=1`) | **Done (2026-04-04)** — `openai-proxy-e2e.spec.js` + `live-mode-openai-proxy.spec.js`: 18 passed, 2 skipped; **`playwright-workers-from-env.cjs`** serial workers |
| Canonical manual repro (host mic + OpenAI proxy) | [MANUAL-REPRO-HOST-MIC-OPENAI-PROXY.md](./MANUAL-REPRO-HOST-MIC-OPENAI-PROXY.md) |
| Manual mic + OpenAI proxy (host audio) | **Re-verify after §2c** — pre-fix captures: **`SHELX.`** etc. in [MANUAL-MIC-OPENAI-PROXY-REPORT-2026-04-08.md](./MANUAL-MIC-OPENAI-PROXY-REPORT-2026-04-08.md); update repro retest table when rerun |
...
| OpenAI proxy commit reschedule (Issue #560) | **Done** — `server.ts`; Jest *reschedules audio commit…* |
| OpenAI proxy **§2c** first-commit + orphan tail (Issue #560) | **Done** — mock: 3× `-t "Issue #560"`; **real API (2026-04-04):** full `openai-proxy-integration.test.ts` with `USE_REAL_APIS=1`, 20 passed (Issue #560 cases mock-only). **Manual mic** still for spoken STT — [COMMIT-TIMING-PROPOSAL.md](./COMMIT-TIMING-PROPOSAL.md) |
| OpenAI proxy **commit scheduler** Phase 1–2 (Issue #560, null-VAD) | **Done (2026-04-04)** — [COMMIT-SCHEDULER-TDD-PLAN.md](./COMMIT-SCHEDULER-TDD-PLAN.md) **§9**; `server.ts` max coalesce + client-close flush; mock: `-t "Issue #560 / scheduler"` + full file `--runInBand --forceExit` green |
| OpenAI proxy **Server VAD** Phase 2b + **default Server VAD** (Issue #560) | **Done** — `translator.ts` defaults **`server_vad`**; **`useOpenAIManualAudioCommit`** Jest-only for null-VAD / proxy `commit` mocks; `server.ts`; Jest *Phase 2b* + full mock integration green |
