...
| OpenAI proxy **§2c** first-commit + orphan tail (Issue #560) | **Done (mock)** — `OPENAI_MIN_AUDIO_BYTES_FOR_FIRST_COMMIT`, silence pad in `rearmPendingAudioCommitAfterResponse`, 3× `-t "Issue #560"` tests; **re-qualify** `USE_REAL_APIS=1` + manual mic — [COMMIT-TIMING-PROPOSAL.md](./COMMIT-TIMING-PROPOSAL.md) |
