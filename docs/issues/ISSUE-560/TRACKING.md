...
| Manual mic + OpenAI proxy (host audio) | **Re-test after proxy fix** — prior log **failing** (2026-04-08): [MANUAL-MIC-OPENAI-PROXY-REPORT-2026-04-08.md](./MANUAL-MIC-OPENAI-PROXY-REPORT-2026-04-08.md); **fix:** `server.ts` **`onResponseEnded`** reschedules **`scheduleAudioCommit`** (+ Jest Issue #560) |
| OpenAI proxy commit reschedule (Issue #560) | **Done** — `packages/voice-agent-backend/scripts/openai-proxy/server.ts`; `tests/integration/openai-proxy-integration.test.ts` |
