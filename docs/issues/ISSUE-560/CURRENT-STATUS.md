...
**Last updated:** 2026-04-04 (**Playwright OpenAI real-API qualification:** **`USE_REAL_APIS=1`** **`openai-proxy-e2e.spec.js`** + **`live-mode-openai-proxy.spec.js`** — 18 passed, 2 skipped; **`workers: 1`** when **`USE_REAL_APIS=1`** — **`test-app/tests/playwright-workers-from-env.cjs`** + Jest **`playwright-workers-from-env.test.js`**. Prior: E2E skip policy (**`e2e-skip-env-policy.cjs`**), backend **`/health`** / **`/ready`**, root **`openai-proxy-integration.test.ts`** 20 passed (2026-04-08); [ISSUE-489-INTEGRATION-OBSERVATIONS.md](./ISSUE-489-INTEGRATION-OBSERVATIONS.md). **Next:** transcript / manual parity — [NEXT-STEP.md](./NEXT-STEP.md).)
