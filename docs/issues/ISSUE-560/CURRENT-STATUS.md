...
- **Proxy audit + schedule:** [COMMIT-SCHEDULER-TDD-PLAN.md](./COMMIT-SCHEDULER-TDD-PLAN.md) **§4** / **§9**; [TDD-PLAN.md](./TDD-PLAN.md) **§2d**. **Phase 1 RED:** `openai-proxy-integration.test.ts` *Issue #560 / scheduler* (×2) **landed** — **failing** until Phase 2 GREEN. **Next:** null-VAD bridge (max coalesce / commit-on-close), then Phase 2b Server VAD, P1/P2 per plan.
