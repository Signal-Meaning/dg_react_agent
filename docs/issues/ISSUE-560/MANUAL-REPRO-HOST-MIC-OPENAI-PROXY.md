...
5. **Browser:** open the app URL (e.g. **`http://localhost:5173`**). Hard-refresh if you changed frontend bundles.  
   **Optional — tie mic intent → permission → first binary → first commit:** append **`micTimingDebug=1`** (e.g. **`http://localhost:5173/?micTimingDebug=1`**). The Event Log will emit one or more **`[micTimingDebug]`** lines with `Date.now()` fields: **`userMicIntentAt`** (Live entry / **Enable Mic** / **Resume microphone**), **`gumInvokedAt`** / **`gumResolvedAt`**, **`firstBinaryWsSendAt`**, **`startAudioCapture*`** deltas. On the same machine, the proxy’s first **`input_audio_buffer.commit + response.create`** log includes **`debug.wall_clock_ms`** (ms epoch); compare to **`firstBinaryWsSendAt`** and expect **`commit`** a few hundred ms **after** the last binary chunk of the first burst (debounce). See **`test-app/src/mic-timing-debug.ts`**.
