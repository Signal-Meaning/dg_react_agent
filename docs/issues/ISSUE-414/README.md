...
- **Test status (after session config: turn_detection null + format):**
  - **Real-API firm audio:** Passes 5/5. **Greeting flow:** Passes with default Settings (idleTimeoutMs 10s). Idle timeout is shared via Settings.agent.idleTimeoutMs (no separate env var). See [CURRENT-UNDERSTANDING.md §2.1](./CURRENT-UNDERSTANDING.md#21-real-api-verification-firm-audio-test) and [NEXT-STEPS.md](./NEXT-STEPS.md).
