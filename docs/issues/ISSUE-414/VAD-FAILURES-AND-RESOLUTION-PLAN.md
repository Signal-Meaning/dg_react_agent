...
|-------|------------|-----------|
| **E2E test 5b** (`openai-proxy-e2e.spec.js`) | Assertion: at least one VAD event (UserStartedSpeaking or UtteranceEnd) should appear in the UI within 15s. **Received: 0.** | Running against live OpenAI proxy + real API; test sends 24 kHz audio (resampled from hello fixture) via `loadAndSendAudioSampleAt24k`. Connection and audio send succeed; no VAD UI updates. |
...
So we have **proxy mapping** and **component behavior** covered by integration/unit tests. We do **not** have automated coverage for: (a) OpenAI actually sending `speech_started` / `speech_stopped`, or (b) session config (e.g. `turn_detection`) required for the API to emit them.
...
1. **Run proxy with debug logging** so every upstream event `type` is visible (e.g. already in place with `debug` or add a single log line for `msg.type`).
2. **Run E2E test 5b** against the live proxy and real OpenAI (same as today).
3. **Inspect logs:** Do we see `input_audio_buffer.speech_started` and/or `input_audio_buffer.speech_stopped` from upstream?
   - **If yes** → Problem is downstream (proxy→client delivery, component, or test-app state). Go to Phase C.
   - **If no** → Problem is upstream (API not emitting VAD for this session/clip). Go to Phase B.

**Integration testability:** None. This step is manual/diagnostic with real API.
