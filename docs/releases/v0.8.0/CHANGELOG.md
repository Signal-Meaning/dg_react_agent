...
- **CI workflow:** Test-and-publish workflow now builds and publishes both packages: `@signal-meaning/voice-agent-react` (root) and `@signal-meaning/voice-agent-backend` (`packages/voice-agent-backend`). Test job validates voice-agent-backend with `npm pack --dry-run`; publish job publishes root then backend (each skips if version exists unless force is set).
