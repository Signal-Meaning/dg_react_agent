# Usage Examples - v0.9.1

v0.9.1 does not introduce new component APIs. Existing examples from the main docs and previous releases (e.g. v0.9.0 proxy mode, function calling) continue to apply.
...
```bash
npm install @signal-meaning/voice-agent-react@0.9.1
```

If you use the backend package:

```bash
npm install @signal-meaning/voice-agent-backend@0.2.1
```
...
- [NEW-FEATURES.md](./NEW-FEATURES.md) — v0.9.1 (patch: backend fix only)
