# Usage Examples - v0.9.2

v0.9.2 does not introduce new component APIs. Existing examples from the main docs and previous releases (e.g. v0.9.1 proxy mode, function calling) continue to apply.
...
```bash
npm install @signal-meaning/voice-agent-react@0.9.2
```

If you use the backend package:

```bash
npm install @signal-meaning/voice-agent-backend@0.2.2
```
...
- [NEW-FEATURES.md](./NEW-FEATURES.md) — v0.9.2 (patch: backend fix only)
