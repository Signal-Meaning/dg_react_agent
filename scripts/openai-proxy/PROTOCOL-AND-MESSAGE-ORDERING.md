...
**"Buffer too small … 0.00ms" vs "server had an error":** These are **distinct** errors. The likely cause of "buffer too small" is a **dual-control race**: with Server VAD enabled (default), the server auto-commits on `speech_stopped` and auto-sends `response.create`; the proxy *also* sends commit + `response.create` after its 400ms debounce. So (1) server commits on speech_stopped and empties the buffer; (2) 400ms later the proxy sends commit → buffer is already empty → "buffer too small 0.00ms"; (3) proxy sends `response.create` → "conversation already has an active response". **Fix:** Use one strategy only. The proxy disables Server VAD via `session.audio.input.turn_detection: null` (GA path; see [REGRESSION-SERVER-ERROR-INVESTIGATION.md](../../docs/issues/ISSUE-414/REGRESSION-SERVER-ERROR-INVESTIGATION.md) Cycle 2) and sends commit + `response.create` manually. The **5-second "server had an error"** is a different issue; VAD config was ruled out by the REGRESSION doc's 4-cycle investigation; the most promising lead is `idle_timeout_ms` (server VAD idle timeout).
