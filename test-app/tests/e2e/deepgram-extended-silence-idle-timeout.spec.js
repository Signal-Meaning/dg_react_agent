...
import { setupAudioSendingPrerequisites, assertConnectionState, waitForAgentGreeting, skipIfOpenAIProxy } from './helpers/test-helpers';
...
test.describe('Deepgram: Extended Silence Idle Timeout Test', () => {
  test('should demonstrate connection closure with >10 seconds of silence', async ({ page, context }) => {
    skipIfOpenAIProxy('VAD/speech detection and idle timeout flow are Deepgram-only; skip when using OpenAI proxy');
